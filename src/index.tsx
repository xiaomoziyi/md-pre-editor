...
export class MdPriview extends React.Component<IMP> {
  render() {
    const { value, anchor = false } = this.props
    return (
      <div className="for-preview-content">
        <div className="for-panel for-editor-preview for-active">
          <div
            className="for-preview for-markdown-preview"
            dangerouslySetInnerHTML={{ __html: marked(value, anchor) }}
          />
        </div>
      </div>
    )
...
export class MdEditor extends React.Component<IP, IS> {
  static defaultProps = {
    lineNum: true,
    onChange: () => { },
    onSave: () => { },
  ...
  reLineNum() {
    ...
    let lineHeight: number = parseFloat(fontSize.replace('px', '')) * 1.6
    lineHeight = parseInt(lineHeight.toString())
    const baseHeight: number = parseInt(((editHeight - 8) / lineHeight).toFixed())
    this.setState({
  ...
  render() {
    ...
    const { value, placeholder, fontSize, disabled, height, style, toolbar, outline, anchor } = this.props
    ...
    return (
      <div className={fullscreen} style={{ height, ...style }}>
        {/* 菜单栏 */}
        {Boolean(Object.keys(newToolbar).length) && (
...
// export default { MdEditor, MdPriview }
